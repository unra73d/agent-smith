...
submit action. The field SHALL use the same dark styling as the app's other
text inputs (same background, border, and text colors) so it is visually
...
- **AND** the field is styled with the app's dark theme, matching the other text inputs
...
search text and restores the full chats list with a single click. The clear
button SHALL be positioned on the same line as the field (inside the input
area, vertically centered) and SHALL only be visible while the field has text.
...
#### Scenario: Clear button is on the same line as the field

- **WHEN** the search field contains text
- **THEN** the clear button is visible on the same line as the field, inside the input area
